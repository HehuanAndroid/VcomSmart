...
public class SlideColorPickerView extends View {
    ...
    private static final int defaultSizeShort = 210; // * 2
    private static final int defaultSizeLong = 420;
    ...
    public SlideColorPickerView(Context context) {
        super(context);
    }

    public SlideColorPickerView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SlideColorPickerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        final TypedArray array = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SlideColorPickerView, defStyleAttr, 0);
        mIndicatorColor = array.getColor(R.styleable.SlideColorPickerView_indicatorColor, Color.WHITE);

        int or = array.getInteger(R.styleable.SlideColorPickerView_orientation, 0);
        orientation = or == 0 ? Orientation.HORIZONTAL : Orientation.VERTICAL;

        mIndicatorEnable = array.getBoolean(R.styleable.SlideColorPickerView_indicatorEnable, true);
    ...
    private void calculBounds() {
        ...
        //banlap: ColorPanelView重新调整 颜色条长宽
        if (orientation == Orientation.HORIZONTAL) {
            l = mLeft + mRadius;
            r = mRight - mRadius;
     ...
     * 如果想设置 alpha ，可以在{@link OnColorPickerChangeListener#onColorChanged(SlideColorPickerView, int)} 回调
    ...
    private void createIndicatorBitmap() {
        ...
        //banlap: ColorPanelView重新调整 指示点
        int r;
        if (orientation == Orientation.HORIZONTAL) {
            r = bitmapForColor.getHeight() / 14;
        } else {
            r = 10;
        }

        int b =mRadius+mRadius/4;

        RectF rf = new RectF(0, mRadius, bitmapForIndicator.getWidth(), b);
        c.drawRoundRect(rf, r, r, paintForIndicator);
    ...
    private void createColorTableBitmap() {
        ...
        // 圆角大小
        int r;
        //banlap: ColorPanelView重新调整 颜色条长宽的圆弧
        if (orientation == Orientation.HORIZONTAL) {
            //r = bitmapForColor.getHeight() / 14;
            r = 10;
        } else {
            //r = bitmapForColor.getWidth() / 14;
            r = 10;
        }
    ...
    private boolean inBoundOfColorTable(int ex, int ey) {
        //banlap: ColorPanelView重新调整 颜色条滑动范围
        if (orientation == Orientation.HORIZONTAL) {
            if (ex <= mLeft + mRadius/3 || ex >= mRight - mRadius/3) {
                return false;
            }
        } else {
            if (ey <= mTop + mRadius/10 || ey >= mBottom - mRadius/3) {
                return false;
            }
        }
        return true;
    ...
    public interface OnColorPickerChangeListener {
         ...
         * @param color  颜色
         */
        void onColorChanged(SlideColorPickerView picker, int color);
         ...
         * @param picker ColorPickerView
         */
        void onStartTrackingTouch(SlideColorPickerView picker);
         ...
         * @param picker ColorPickerView
         */
        void onStopTrackingTouch(SlideColorPickerView picker);
